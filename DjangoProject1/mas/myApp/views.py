...
def linear_solver(world,search_strategy='breadth_first'):
    state = []
    ...
    if search_strategy == 'depth_first':
        return depth_first_search(world, state, goals, [])
    elif search_strategy == 'breadth_first':
        return breadth_first_search(world, state, goals)
    
def depth_first_search(world, state, goals, current_plan, depth=0):
    ...
    while i < len(goals):
        ...
        if satisfied(state, goal):
            ...
            for action in possible_actions:
                ...
                solution = depth_first_search(world, temp_state, subgoals, current_plan, depth=depth + 1)
...
def breadth_first_search(world, state, goals):
    # Breadth-first search implementation using a queue
    queue = Queue()
    queue.put((state, goals, []))

    while not queue.empty():
        current_state, current_goals, current_plan = queue.get()

        if len(current_goals) == 0:
            return current_plan

        for action in get_possible_grounds(world, current_goals[0]):
            if preconditions_reachable(world, action):
                temp_state = list(current_state)
                temp_goals = list(current_goals)
                temp_plan = list(current_plan)
                ...
                queue.put((temp_state, temp_goals, temp_plan))
...
def home(request):
    ...
    if request.method == 'POST' and request.FILES['file']:
        ...
        if not already_solved:
            ...
            solution_depth_first = linear_solver(w, search_strategy='depth_first')

            print("Solving with Breadth-First Search...")
            solution_breadth_first = linear_solver(w, search_strategy='breadth_first')
        ...
        print(solution_depth_first[0])
        s=solution_depth_first[0]
        ...
        # Print or use the first line as needed
        ss_list = []
        lines_list = []
        first_lines_list = []

        for i in range(len(solution_depth_first)):
            s = solution_depth_first[i]
            ...
            first_lines_list.append(first_line)



        print(first_lines_list)
        ######################################################
        ...
        for j in range(len(solution_breadth_first)):
            s1 = solution_breadth_first[j]

            ss1 = str(s1)
            lines1 = ss1.split('\n')
            first1_line = lines1[0].strip()

            # Append the values to their respective lists
            ss1_list.append(ss1)
            lines1_list.append(lines1)
            first1_lines_list.append(first1_line)
        ...
        result = ' -> '.join(first_lines_list)
        result1 = ' -> '.join(first1_lines_list)
        ...
        context = {
            ...
            'solution_depth_first': result,
            'solution_breadth_first': result1,
        }
